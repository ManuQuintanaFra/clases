...
@Component({
        ...
        <div class="favoritos-grid">
          @for (fav of favoritos(); track fav.id) {
            <div class="favorito-card">
              <!-- Los datos del libro vienen del JOIN con la tabla libros -->
              <img [src]="fav.libros?.portada_url" [alt]="fav.libros?.titulo" class="cover">
              <div class="info">
                <h3>{{ fav.libros?.titulo }}</h3>
                <p class="autor">✍️ {{ fav.libros?.autor }}</p>
                <p class="genero">📂 {{ fav.libros?.genero }}</p>
                
                <!-- Estado del stock en tiempo real (gracias al JOIN) -->
                @if (fav.libros) {
                  <p class="stock" [class.agotado]="!fav.libros.disponible">
                    {{ fav.libros.disponible 
                      ? '✅ ' + fav.libros.ejemplares_restantes + ' disponibles' 
                      : '❌ Agotado' }}
                  </p>
                }
  ...
  styles: [`
    ...
    .autor { color: #7f8c8d; font-size: 0.85em; margin: 2px 0; }
    .genero { color: #95a5a6; font-size: 0.8em; margin: 2px 0; }
    .stock { font-size: 0.85em; font-weight: bold; color: #27ae60; margin: 4px 0; }
